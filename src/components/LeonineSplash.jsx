      ...
      {[
        ...
        <div
          ...
          style={{
            ...
            height: `${m.h}%`,
            background:
              "linear-gradient(90deg, rgba(180,200,170,0.13) 0%, rgba(180,200,170,0.04) 60%, transparent 100%)",
            opacity: m.op,
      ...
      {[
        ...
        <div
          ...
          style={{
            ...
            height: `${m.h}%`,
            background:
              "linear-gradient(270deg, rgba(180,200,170,0.13) 0%, rgba(180,200,170,0.04) 60%, transparent 100%)",
            opacity: m.op,
            animation: `mistDrift ${m.dur}s ease-in-out ${m.del}s infinite alternate`,
            pointerEvents: "none",
          }}
...
export default function LeonineSplash({ onComplete }) {
  ...
  const handleEnter = () => {
    // Phase 6 = exit: fade entire splash to black, then call onComplete
    setPhase(6);
  ...
  return (
    <>
      {/* ── Keyframe styles ── */}
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,500;0,700;1,400&family=Cormorant+Infant:ital,wght@0,300;1,300&family=Jost:wght@200;300;400;500&display=swap');
      ...
      {/* ══ ROOT — covers entire viewport, fades to black on exit ══ */}
      <div
        style={{
          position: "fixed",
          inset: 0,
          ...
          // Fade the WHOLE splash to near-black (matches the dark home bg) before unmount
          opacity: exiting ? 0 : 1,
          background: "#050e06",
      ...
      >
        ...
        <div
          style={{
            ...
            inset: 0,
            background:
              "radial-gradient(ellipse at 50% 40%, #0d2410 0%, #070f08 45%, #030806 100%)",
            opacity: phase >= 1 ? 1 : 0,
        ...
        <div
          style={{
            ...
            inset: 0,
            backgroundImage:
              "repeating-linear-gradient(0deg, transparent, transparent 39px, rgba(201,168,76,0.025) 40px)",
            pointerEvents: "none",
          }}
        />
        ...
        >
          ...
          <line x1="24" y1="24" x2="976" y2="24"
            stroke="url(#borderGrad)" strokeWidth="0.4"
            strokeDasharray="952" strokeOpacity="0.45"
            strokeDashoffset={phase >= 1 ? 0 : 952}
            style={{ transition: "stroke-dashoffset 2s cubic-bezier(0.4,0,0.2,1) 0.8s" }}
          />
          <line x1="24" y1="576" x2="976" y2="576"
            stroke="url(#borderGrad)" strokeWidth="0.4"
            strokeDasharray="952" strokeOpacity="0.45"
            strokeDashoffset={phase >= 1 ? 0 : 952}
            style={{ transition: "stroke-dashoffset 2s cubic-bezier(0.4,0,0.2,1) 0.9s" }}
          />
          <line x1="24" y1="24" x2="24" y2="576"
            stroke="url(#borderGradV)" strokeWidth="0.4"
            strokeDasharray="552" strokeOpacity="0.45"
            strokeDashoffset={phase >= 1 ? 0 : 552}
            style={{ transition: "stroke-dashoffset 1.8s cubic-bezier(0.4,0,0.2,1) 0.85s" }}
          />
          <line x1="976" y1="24" x2="976" y2="576"
            stroke="url(#borderGradV)" strokeWidth="0.4"
            strokeDasharray="552" strokeOpacity="0.45"
            strokeDashoffset={phase >= 1 ? 0 : 552}
          ...
          {phase >= 1 && (
            <rect x="496" y="8" width="8" height="8" fill="#c9a84c"
              transform="rotate(45 500 12)" opacity="0.9" />
          )}
          {/* Bottom centre diamond */}
          {phase >= 1 && (
            <rect x="496" y="584" width="8" height="8" fill="#c9a84c"
              transform="rotate(45 500 588)" opacity="0.9" />
          )}
          {/* Left / Right centre diamonds */}
          {phase >= 1 && (
            <>
              <rect x="8" y="296" width="8" height="8" fill="#c9a84c"
                transform="rotate(45 12 300)" opacity="0.9" />
              <rect x="984" y="296" width="8" height="8" fill="#c9a84c"
                transform="rotate(45 988 300)" opacity="0.9" />
            </>
          )}

          {/* Inner border corner accent diamonds */}
          {phase >= 1 &&
            [[24, 24], [976, 24], [24, 576], [976, 576]].map(([cx, cy], i) => (
              <rect key={i} x={cx - 3} y={cy - 3} width="6" height="6" fill="#c9a84c"
                transform={`rotate(45 ${cx} ${cy})`} opacity="0.5" />
            ))}

          {/* Quarter-point accent diamonds on outer border top/bottom */}
          {phase >= 1 &&
            [250, 750].map((cx, i) => (
