...
export default function AdminBooking() {
  ...
  const statusBadge = (status) => {
    if (status === "Confirmed")
      return "text-teal-700 bg-teal-50 border border-teal-100";
    if (status === "Cancelled")
    ...
    >
                  ...
                  currentBookings.map((b, index) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center">
                        <span
                          className={`text-xs font-semibold px-2.5 py-1 rounded-full ${statusBadge(b.status)}`}
                        >
                          {b.status || "Pending"}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <button
                            onClick={() =>
                              navigate("/admin/bookings/update", { state: b })
                            }
                            title="Edit booking"
      ...
      {totalPages > 1 && (
        <div className="bg-white border-t border-slate-200 px-6 py-3 flex items-center justify-between">
          <p className="text-xs text-slate-400">
            Showing {indexOfFirst + 1}–{Math.min(indexOfLast, bookings.length)}{" "}
            of {bookings.length}
          </p>
