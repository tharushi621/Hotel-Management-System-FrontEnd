...
// Each room card maps to a backend "category" value
const ROOMS = [
  { id: "canopy",   category: "canopy",   label: "The King's Canopy Suite",  icon: "🌿", price: "from $480 / night" },
  { id: "ganga",    category: "ganga",    label: "Ganga Riverside Villa",    icon: "💧", price: "from $380 / night" },
  { id: "heritage", category: "heritage", label: "The Heritage Forest Lodge", icon: "🏛️", price: "from $320 / night" },
];
...
export default function BookingPage() {
  ...
  const [bookingRef, setBookingRef] = useState(null); // returned booking object
  ...
  function handleSubmit() {
    ...
    // Uses createBookingUsingCategory endpoint.
    // Backend auto-generates bookingId and assigns roomId from the category.
    // req.user.email is taken from the JWT — no need to send email in the body.
    axios
      .post(
        import.meta.env.VITE_BACKEND_URL + "/api/bookings/category",
        {
          ...
          // Pack guest-facing info into notes so admin can see it
          notes: [
            `Name: ${form.name}`,
      ...
      .catch((err) => {
        setError(
          err.response?.data?.message ||
            "The courier has faltered — please try again presently."
        );
        setLoading(false);
      });
  }
