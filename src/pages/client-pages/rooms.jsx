...
export default function RoomsPage() {
      ...
      try {
        if (categoryId) {
          const r1 = await axios.get(
            `${base}/api/rooms?category=${categoryId}`,
          );
          const l1 = extract(r1.data);
          if (l1.length > 0) {
            if (!categoryName && l1[0]?.category)
              setDisplayName(l1[0].category);
            setRooms(l1);
            setLoading(false);
            return;
          }
        ...
        if (categoryName) {
          const r2 = await axios.get(
            `${base}/api/rooms?name=${encodeURIComponent(categoryName)}`,
          );
  ...
  return (
    <div className="font-body bg-stone-950 overflow-x-hidden">
      ...
      {lightboxImg && (
        <div className="lightbox" onClick={() => setLightboxImg(null)}>
          <div className="relative max-w-5xl max-h-[90vh] mx-4">
            ...
            />
            <button className="absolute top-4 right-4 text-stone-300 hover:text-white text-2xl bg-black/50 w-10 h-10 rounded-full flex items-center justify-center">
              ✕
            </button>
          </div>
        </div>
        ...
        <div className="max-w-screen-xl mx-auto px-6 flex items-center justify-between">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              />
              <span className="font-display text-2xl font-medium tracking-widest text-amber-100 hover:text-yellow-400 transition-colors">
                LEONINE
              </span>
            </div>
            <span className="font-body text-xs tracking-[0.4em] text-yellow-600/70 ml-10 uppercase">
              Villa & Sanctuary
            </span>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-3">
            <button
              className="btn-outline font-body text-xs tracking-widest uppercase px-4 py-2 rounded-full"
          ...
          <div className="lg:hidden nav-carved border-t border-yellow-900/30 py-6 px-6">
            {["Home", "Retreats", "Gallery", "Contact"].map((item) => (
              <a
                ...
                className="block font-body text-xs tracking-widest uppercase text-stone-300 hover:text-yellow-400 py-3 border-b border-stone-800/50"
            ...
            <button
              className="mt-4 w-full btn-gold font-body text-xs tracking-widest uppercase py-3 rounded-full text-stone-900 font-medium"
              onClick={() => {
                setMenuOpen(false);
                navigate("/booking");
              }}
      ...
      <section className="relative h-[65vh] min-h-[480px] flex items-end overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-screen-xl mx-auto px-6 pb-16">
          <div className="anim-in flex items-center gap-2 mb-6 text-stone-500">
            <button
              onClick={() => navigate("/")}
              className="font-body text-xs tracking-wider uppercase hover:text-yellow-500 transition-colors"
            >
              Home
            </button>
            <span>/</span>
            ...
            <span>/</span>
            <span className="font-body text-xs tracking-wider uppercase text-yellow-600">
              {displayName}
            </span>
          </div>
          ...
          <h1 className="font-display leading-none">
            <span className="block text-5xl md:text-7xl font-medium hero-text-mask">
              {displayName}
            </span>
            <span className="block text-3xl md:text-4xl font-light italic anim-shimmer mt-2">
              Rooms & Villas
            </span>
          </h1>
          <p className="font-serif-light text-lg md:text-xl text-stone-300 italic font-light max-w-lg mt-5 leading-relaxed">
            Each room is a world unto itself — handcrafted, storied, and alive
            with the spirit of Lanka.
          </p>
        </div>
      ...
      <main
        style={{ background: "linear-gradient(180deg,#0a160c,#0d1a10)" }}
      ...
      >
        ...
        {!loading && !error && rooms.length > 0 && (
          <div className="max-w-screen-xl mx-auto px-6 mb-12 flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-4">
              <div
                style={{ height: "1px", width: "32px", background: "#c9a84c" }}
              ></div>
            ...
            <button
              onClick={() => navigate("/booking")}
              className="btn-gold font-body text-xs tracking-widest uppercase px-8 py-3 rounded-full text-stone-900 font-medium"
            >
              Check Availability →
            </button>
          </div>
        ...
        {!loading && error && (
          <div className="max-w-screen-xl mx-auto px-6 py-20 text-center">
            ...
            <h3 className="font-display text-2xl text-amber-100 mb-3">
              Could Not Load Rooms
            </h3>
            <p className="font-body text-xs text-stone-600 mb-8 italic">
              {error}
            </p>
            <div className="flex gap-4 justify-center flex-wrap">
              ...
              <button
                className="btn-outline font-body text-xs tracking-widest uppercase px-8 py-3 rounded-full"
                onClick={() => navigate("/booking")}
              >
                Book Directly
              </button>
        ...
        {!loading &&
          ...
          rooms.map((room, index) => {
            ...
            const category =
              room.category || room.categoryName || room.type || displayName;
            const roomNum =
              room.roomId || room.roomNumber || room.number || index + 1;
                  ...
                  <div className="grid lg:grid-cols-2">
                    <div className="relative h-80 lg:h-auto min-h-[400px] overflow-hidden">
                      ...
                      <div className="absolute top-5 left-5">
                        <span
                          className="font-body text-xs tracking-widest uppercase px-4 py-2 rounded-full text-stone-900 font-medium"
                          style={{
                            background:
                      ...
                      {photos.length > 1 && (
                        <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex items-center gap-3">
                          <button
                            className="slider-btn text-stone-300 hover:text-yellow-400"
                            onClick={() => cycleImg(id, photos.length, -1)}
                          >
                            ‹
                          </button>
                    ...
                    <div className="p-8 md:p-10 flex flex-col justify-between">
                      <div>
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          ></div>
                          <span className="font-body text-xs tracking-[0.35em] uppercase text-yellow-600/80">
                            {category}
                          </span>
                        </div>
                      ...
                      <div>
                        ...
                        >
                          <div className="font-body text-xs tracking-widest uppercase text-stone-500 mb-2">
                            Every stay includes
                          </div>
                          <div className="flex flex-wrap gap-x-4 gap-y-1">
                            {[
                              ...
                              <span
                                key={inc}
                                className="font-body text-xs text-stone-400 flex items-center gap-1"
                              >
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                          <button
                            ...
                            onClick={() =>
                              navigate(
                                `/booking?room=${roomNum}&category=${encodeURIComponent(category)}`,
                              )
                            }
                          >
                            {isAvail
                          ...
                          <button
                            className="btn-outline flex-1 font-body text-xs tracking-widest uppercase py-4 rounded-full"
                            onClick={() => navigate("/booking")}
                          >
                            Check Availability
                          </button>
        ...
        {!loading && !error && rooms.length === 0 && (
          <div className="max-w-screen-xl mx-auto px-6 py-32 text-center">
            <h3 className="font-display text-3xl text-amber-100 mb-4">
              No Rooms Found
            </h3>
            <p className="font-serif-light text-xl italic text-stone-400 mb-3">
              Rooms for <span style={{ color: "#c9a84c" }}>{displayName}</span>{" "}
              are coming soon.
            </p>
            <p className="font-body text-xs text-stone-600 mb-10 tracking-wider">
              Or contact our team — we'll curate the perfect stay for you.
            </p>
            <div className="flex gap-4 justify-center flex-wrap">
              <button
                className="btn-gold font-body text-xs tracking-widest uppercase px-10 py-4 rounded-full text-stone-900 font-medium"
                onClick={() => navigate("/retreats")}
              >
                Explore All Retreats
              </button>
              <button
                className="btn-outline font-body text-xs tracking-widest uppercase px-10 py-4 rounded-full"
                onClick={() => navigate("/booking")}
              >
                Make a Reservation
              </button>
        ...
        {!loading && !error && rooms.length > 0 && (
          <div className="max-w-screen-xl mx-auto px-6 mt-8 flex justify-center">
            <button
              className="btn-outline font-body text-xs tracking-widest uppercase px-8 py-3 rounded-full flex items-center gap-2"
              onClick={() => navigate("/retreats")}
            >
      ...
      >
        ...
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="font-display text-5xl md:text-6xl text-amber-50 font-medium mb-3">
            Your Room
          </h2>
          ...
          <p className="font-serif-light text-xl italic text-stone-400 font-light mb-12 max-w-xl mx-auto leading-relaxed">
            Only 6 retreats available at any one time. Each stay is personally
            curated.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <button
              className="btn-outline font-body text-sm tracking-widest uppercase px-12 py-5 rounded-full"
              onClick={() => (window.location.href = "tel:+94812204455")}
            >
              Speak to a Curator
            </button>
