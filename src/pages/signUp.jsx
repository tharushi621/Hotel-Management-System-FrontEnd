  ...
  return (
    <>
      <style>{`
        ...
        .letter-outer { position:relative; width:min(560px,94vw); animation:letterDrop 0.9s cubic-bezier(.16,1,.3,1) both; }
        ...
        .parchment {
          position:relative;
          background: radial-gradient(ellipse 45% 35% at 12% 14%,#c8a96844 0%,transparent 60%),
        ...
        .fold-line { position:absolute; top:28%; left:0; right:0; height:1px; pointer-events:none;
          background:linear-gradient(90deg,transparent 4%,#b8904433 20%,#a07a3844 50%,#b8904433 80%,transparent 96%); }
        .fold-shadow { position:absolute; top:calc(28% + 1px); left:0; right:0; height:4px; pointer-events:none;
          background:linear-gradient(to bottom,#00000011,transparent); }
        .postmark { position:absolute; top:22px; right:28px; width:72px; height:72px; border:2px solid #8b4513aa;
          border-radius:50%; display:flex; flex-direction:column; align-items:center; justify-content:center;
          transform:rotate(12deg); opacity:0.6; pointer-events:none; }
        .postmark-inner { width:58px; height:58px; border:1px solid #8b4513aa; border-radius:50%;
        ...
        .stamp-corner { position:absolute; top:18px; right:110px; width:42px; height:52px; border:1.5px solid #8b4513aa;
          padding:2px; opacity:0.55; pointer-events:none; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:2px; }
        .stamp-inner { border:1px solid #8b451388; width:100%; height:100%; display:flex; flex-direction:column; align-items:center; justify-content:center; }
        ...
        .wax-seal { position:absolute; bottom:-28px; right:36px; width:80px; height:80px; z-index:10;
        ...
        .letter-heading { font-family:'Playfair Display',serif; font-size:clamp(1.45rem,4.5vw,1.95rem); font-weight:700; color:var(--ink);
          text-align:center; margin:10px 0 4px; letter-spacing:0.06em; line-height:1.15; position:relative; z-index:1; }
        .quill-divider { display:flex; align-items:center; gap:8px; margin:8px 0 16px; position:relative; z-index:1; }
        ...
        .field-row { display:grid; grid-template-columns:1fr 1fr; gap:0 20px; position:relative; z-index:1; }
        @media(max-width:480px){.field-row{grid-template-columns:1fr}}
        .field-label { font-family:'IM Fell English',serif; font-style:italic; font-size:0.74rem; color:var(--ink-faded);
          margin-bottom:3px; display:block; letter-spacing:0.04em; position:relative; z-index:1; }
        ...
        .ink-field { width:100%; background:transparent; border:none; border-bottom:1.5px solid #a07838cc; outline:none;
          ...
          padding:4px 28px 5px 2px; transition:border-color 0.3s; position:relative; z-index:1;
        ...
        .pw-toggle { position:absolute; right:2px; bottom:5px; background:none; border:none; padding:0;
          cursor:pointer; display:flex; align-items:center; color:#a07838aa; transition:color 0.2s,transform 0.15s; z-index:2; line-height:1; }
        ...
        .seal-btn { width:100%; padding:13px 0; margin-top:6px;
        ...
        .consent-note { font-family:'IM Fell English',serif; font-style:italic; font-size:0.7rem; color:#8b6030aa; margin-bottom:12px; position:relative; z-index:1; line-height:1.5; }
        .letter-ps { font-family:'IM Fell English',serif; font-style:italic; font-size:0.78rem; color:var(--ink-faded); margin-top:18px; text-align:center; position:relative; z-index:1; line-height:1.6; }
        .letter-ps button { background:none; border:none; color:var(--seal-red); font-family:'IM Fell English',serif; font-style:italic; font-size:inherit; cursor:pointer; padding:0; text-decoration:underline; text-decoration-color:#8b1a1a55; text-underline-offset:3px; transition:color 0.2s; }
        ...
        @media(max-width:500px){ .parchment{padding:36px 24px 52px} .postmark{display:none} .stamp-corner{display:none} }
      `}</style>
      ...
      <div className="signup-bg">
        ...
        <div className="letter-outer">
          ...
          <div className="parchment">
            ...
            <p className="salutation">Dear Esteemed Traveller, pray inscribe your particulars below, that we may duly record your name amongst our honoured company.</p>
                  ...
                  {[
                    ...
                    <div key={f.name}>
                      ...
                      <div className={`field-wrap ${inkFocus===f.name?"focused":""}`}>
                        ...
                        <input type="text" name={f.name} placeholder={f.placeholder} className="ink-field no-pr"
                          value={form[f.name]} onChange={handleChange}
                          onFocus={()=>setInkFocus(f.name)} onBlur={()=>setInkFocus(null)} autoComplete={f.name} />
                      </div>
                    </div>
                  ))}
                </div>

                <label className="field-label">Electronic Correspondence<span className="req"> *</span></label>
                <div className={`field-wrap ${inkFocus==="email"?"focused":""}`}>
                  <div className="ink-dot" />
                  <input type="email" name="email" placeholder="your.name@correspondence.com" className="ink-field no-pr"
                    value={form.email} onChange={handleChange}
                    onFocus={()=>setInkFocus("email")} onBlur={()=>setInkFocus(null)} autoComplete="email" />
                </div>
                ...
                <div className="field-row">
                  {[
                    ...
                    <div key={f.name}>
                      ...
                      <div className={`field-wrap ${inkFocus===f.name?"focused":""}`}>
                        <div className="ink-dot" />
                        <input type="tel" name={f.name} placeholder={f.placeholder} className="ink-field no-pr"
                          value={form[f.name]} onChange={handleChange}
                          onFocus={()=>setInkFocus(f.name)} onBlur={()=>setInkFocus(null)} autoComplete="tel" />
                      </div>
                    </div>
                  ))}
                </div>
