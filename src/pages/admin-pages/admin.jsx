...
async function fetchRecentBookings() {
  try {
    ...
    const data = await res.json();
    const arr =
      data.result ?? data.bookings ?? (Array.isArray(data) ? data : []);
...
export default function AdminPage() {
    ...
    (async () => {
      ...
      const [bookings, rooms, users, rating, recent] = await Promise.all([
        ...
        fetchStat("/api/feedbacks", (d) => {
          ...
          const avg =
            arr.reduce((s, f) => s + (Number(f.rating) || 0), 0) / arr.length;
  ...
  const adminFullName = adminLastName
    ...
    >
      ...
      <div className="sb" style={{ width: collapsed ? "66px" : "236px" }}>
        ...
        <div style={{ padding: "10px 10px 0" }}>
          <button
            ...
            className={`ni ${isOnDashboard ? "act" : ""}`}
            title={collapsed ? "Dashboard" : undefined}
          >
            <span
              className="ni-icon"
              style={{ color: isOnDashboard ? "#c9a84c" : "inherit" }}
            >
              <FaHome />
            </span>
          ...
          >
            <span
              className="ni-icon"
              style={{ color: "rgba(124,173,122,0.75)" }}
            >
              ↗
            </span>
            {!collapsed && (
        ...
        <nav style={{ flex: 1, overflowY: "auto", padding: "0 10px" }}>
          {!collapsed && (
            <div className="sec-label" style={{ padding: "0 4px 6px" }}>
              Modules
            </div>
          )}
          {navItems.map((item) => (
            <Link
              key={item.to}
              to={item.to}
              className={`ni ${isActive(item.to) ? "act" : ""}`}
              title={collapsed ? item.label : undefined}
            >
              <span
                className="ni-icon"
                style={{ color: isActive(item.to) ? item.accent : "inherit" }}
              >
                {item.icon}
              </span>
        ...
        >
          <div
            style={{
              ...
              borderRadius: "12px",
              padding: collapsed ? "9px 7px" : "9px 13px",
              display: "flex",
              alignItems: "center",
              gap: "9px",
            }}
          ...
          <div
            style={{
              background:
                "linear-gradient(135deg,rgba(201,168,76,0.08),rgba(26,58,30,0.4))",
          ...
          >
            ...
            {!collapsed && (
              <>
                ...
                >
                  <FaSignOutAlt
                    style={{ color: "rgba(201,168,76,0.3)", fontSize: "11px" }}
                  />
                </button>
              </>
            )}
      ...
      <div className="mca">
        <div className="top-bar">
          <div className="bc">
            Leonine Admin &nbsp;/&nbsp; <span>{activeLabel}</span>
          </div>
            ...
            <>
              <div className="wb fi">
                <div style={{ position: "relative", zIndex: 1 }}>
                  ...
                  >
                    Live data from your MongoDB database. All stats update on
                    each visit.
                  </p>
                </div>
              </div>
              ...
              >
                {quickStats.map((s, i) => (
                  <div key={i} className="stat-card">
                    ...
                    <div
                      className={`stat-num ${loadingStats ? "pulsing" : ""}`}
                    >
                      {s.value}
                    </div>
                    <div
              ...
              >
                <div className="ra-card">
                  ...
                  >
                    <div className="sec-label" style={{ marginBottom: "3px" }}>
                      Quick Access
                    </div>
                    ...
                    <Link key={i} to={item.to} className="ra-row">
                      <div
                        style={{
                          padding: "6px",
                          borderRadius: "7px",
                          background: `rgba(${item.accent === "#c9a84c" ? "201,168,76" : item.accent === "#7cad7a" ? "124,173,122" : "212,137,26"},0.1)`,
                          color: item.accent,
                          fontSize: "12px",
                        }}
                      >
